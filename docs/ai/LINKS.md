# Verified Links

**Last checked:** 2026-08-15

Использовать только официальные источники. Перед platform feasibility gate заново
проверить документацию, permissions, review flow и дату актуальности.

## Product platforms

- [Instagram content publishing](https://developers.facebook.com/docs/instagram-platform/content-publishing/)
- [TikTok Content Posting API](https://developers.tiktok.com/doc/content-posting-api-get-started/)
- [TikTok Direct Post](https://developers.tiktok.com/doc/content-posting-api-reference-direct-post/)
- [TikTok sharing guidelines](https://developers.tiktok.com/doc/content-sharing-guidelines/)
- [YouTube upload guide](https://developers.google.com/youtube/v3/guides/uploading_a_video)
- [YouTube resumable uploads](https://developers.google.com/youtube/v3/guides/using_resumable_upload_protocol)
- [YouTube videos resource](https://developers.google.com/youtube/v3/docs/videos)

VK Clips и Pinterest links добавляются только при подготовке этапа 2.

## Frontend

- [React 18](https://react.dev/)
- [Vite 6](https://v6.vite.dev/guide/)
- [React Router 6](https://reactrouter.com/6.30.1/start/overview)
- [TanStack Query 5](https://tanstack.com/query/v5/docs/framework/react/overview)
- [Tailwind CSS 3](https://v3.tailwindcss.com/docs/installation)
- [Web Push on iOS and iPadOS](https://webkit.org/blog/13878/web-push-for-web-apps-on-ios-and-ipados/)
- [Service Worker API](https://developer.mozilla.org/docs/Web/API/Service_Worker_API)
- [IndexedDB API](https://developer.mozilla.org/docs/Web/API/IndexedDB_API)

## Backend and data

- [Node.js 20 documentation](https://nodejs.org/docs/latest-v20.x/api/)
- [Express 4 API](https://expressjs.com/en/4x/api.html)
- [Prisma ORM documentation](https://www.prisma.io/docs/orm)
- [Prisma Migrate](https://www.prisma.io/docs/orm/prisma-migrate)
- [PostgreSQL 16](https://www.postgresql.org/docs/16/)
- [BullMQ](https://docs.bullmq.io/)
- [Redis 7](https://redis.io/docs/latest/)
- [Cloudflare R2 S3 API](https://developers.cloudflare.com/r2/api/s3/api/)

## Repository sources

- [Product requirements](../product-requirements.md)
- [Architecture decision](../adr/0001-architecture.md)
- [Platform feasibility](../platform-feasibility.md)
- [Prisma schema](../../backend/prisma/schema.prisma)
- [Workspace manifest](../../package.json)
